...
/*                                                                                                  bonusbonus
void rez_generatie_bonus(char *matr, int col, int lin, Node_Conex **top, FILE *bonus)
{
    ...
    scriere_f_out(bonus, matr, lin, col);
...
void createGfromVector(Poz const *elemente_graph, int vii, GRAPH *g)
{
    g->V = vii;
    g->E = 0;
...
void preord_rez_gen4task4(T_Node *root, int lin, int col, char *grila, FILE *f_out) //pentru task4
{
    Poz *elemente_graph;
        GRAPH *g;
        int vii = inViata(grila, lin, col, &elemente_graph);
        createGfromVector(elemente_graph, vii, g);

        Node_Conex *top_conex;
        int nr_comp_conexe = DFS(g, &top_conex);

        int nr_max_path = -1;
        int *max_path = NULL;
        while(!isEmpty_conex(top_conex))
        {
            Node *aux = pop_conex(&top_conex);
            int *de_vizitat, *path;
            int nr_de_vizitat = convertStack2Vector(&aux, &de_vizitat);
            if(hamilton(g, de_vizitat, nr_de_vizitat, &path) == 1)
            {
                if(nr_max_path < nr_de_vizitat)
                {
                    nr_max_path = nr_de_vizitat;
                    cpy_vector(&max_path, &path, nr_de_vizitat);
                }
            }
    ...
    {
        fprintf(f_out, "%d\n", nr_max_path-1);
        for(int i=0; i<nr_max_path; i++)
        {
            fprintf(f_out, "%d,%d ", elemente_graph[max_path[i]].linie, elemente_graph[max_path[i]].coloana);
        }
        fprintf(f_out, "\n");
    }
    ...
    if(root->left)
    {
        ...
        preord_rez_gen4task4(root->left, lin, col, grila1, f_out);
        preord_rez_gen4task4(root->right, lin, col, grila2, f_out);
    }
    else if (root->generatie > 0) 
            free(grila);
